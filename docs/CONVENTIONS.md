...
1. **QED2-CONTRACT.md** — three-party integration contract. Wins every conflict. (Not in this repo; ask the maintainer for the current copy.)
...
4. **UI prototype** — visual truth (tokens, layouts). Function defers to the supplement. (Not in this repo.)
...
Test vectors in `packages/core-logic/test/fixtures/checksum-vectors.json` are generated by the spec §7 reference implementation verbatim (node:crypto oracle). Cross-validation against the live server is covered by `test/e2e-live.spec.ts` (needs a dedicated test account; see README).
...
- **Mastery**: per competency EMA — `m' = m + 0.3·(ratio − m)`, initialized to `ratio` on first grade; `ratio = awardedPoints/maxPoints` of the graded part (`fsrs/mastery.ts`). Each graded part updates every competency attached to that part.
...
- Deploy target: GitHub Pages, custom domain `qed.barcarolle.studio` (SPA fallback 404.html, `base: '/'` — served from the domain root, not a relative base).
