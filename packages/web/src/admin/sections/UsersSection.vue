...
async function openDetail(user: User) {
  ...
  await run((signal) => props.client.request<UserDetail>(`/users/${encodeURIComponent(user.id)}`, undefined, true, undefined, signal), (value) => { detail.value = value; });
}
async function openAction(user: User, operation: typeof action.value) {
