...
async function openDetail(invite: Invite) {
  if (!can('inviteUsage')) return;
  detail.value = undefined; generated.value = '';
  await run((signal) => props.client.request<Invite>(`/invites/${encodeURIComponent(invite.id)}`, undefined, true, undefined, signal), (value) => { detail.value = value; });
}
function openAction(invite: Invite, value: typeof action.value) { if (!can(value === 'delete' ? 'inviteDelete' : 'inviteEdit')) return; target.value = invite; action.value = value; expires.value = dateInput(invite.expiresAt); error.value = ''; generated.value = ''; }
