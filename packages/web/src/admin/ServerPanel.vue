...
async function saveQuota(): Promise<void> {
  await run(async () => {
    ...
    selectedUser.value = undefined; notice.value = 'AI 授权已保存，已用额度保持不变。'; await load();
  });
}
...
<template>
  ...
  <template v-if="tab === 'overview'">
    ...
    <p class="field-hint">练习统计仅包含服务端收到的已登录账号上报，可能遗漏游客、离线及尚未同步的活动，不代表全部日活。</p>
    <section v-if="info" class="surface"><h3>服务信息</h3><dl class="detail-list"><dt>Server 版本</dt><dd>{{ info.version ?? '—' }}</dd><dt>代码提交</dt><dd class="mono">{{ info.commit ?? '—' }}</dd><dt>数据库</dt><dd>{{ info.database?.connected === true || info.database?.status === 'connected' ? '已连接' : info.database?.connected === false || info.database?.status === 'down' ? '未连接' : '—' }}</dd><dt>共享池费用 · 近 7 天</dt><dd>{{ count(stats?.ai.poolCostCentsLast7Days) }} 美分</dd></dl>
      ...
      <div v-if="restartConfirm" class="confirmation"><h4>重启此 Server 节点？</h4><p>登录、同步和 AI 服务会短暂中断，Core 继续独立运行。服务由进程管理器重新启动后，需要重新登录管理界面。</p><div class="actions"><button type="button" class="danger" :disabled="busy" @click="restart">确认重启</button><button type="button" @click="restartConfirm = false">取消</button></div></div>
    </section>
  </template>
  ...
  <template v-if="tab === 'usage'">
    ...
    <p class="field-hint">按 UTC 日期汇总。用户自己的密钥费用仅为估算值。</p>
  ...
  <template v-if="tab === 'audit'">
    <p class="field-hint">记录数据库管理操作。节点认证、密码恢复与重启意图另记在节点本机审计文件中。</p>
