...
describe('ProgressView information density', () => {
  it('uses concise detail titles and helper copy', () => {
    ...
    expect(progressSource).toContain('return `Aktivität · ${formatDayKey(selectedActivityDate.value)}`;');
    ...
    expect(progressSource).toContain('{{ gradedPartCount }} Teile');
    expect(progressSource).toContain('<span>Durchschnitt</span>');
