...
describe('progress sync orchestration', () => {
  ...
  it('exposes a readable error while retaining attempts rejected by the server', async () => {
    const progress = await setup();
    ...
    expect(progress.attemptUploadStatus).toEqual({
      state: 'error',
      pendingCount: 1,
      message: 'Der Antwortverlauf konnte nicht hochgeladen werden. Bitte erneut versuchen.',
    });
  });
