...
watch(
  ...
  (detent) => {
    ...
    // Returning to the answer preview must not retain a scroll position from
    // the marking notes or assessment in the expanded drawer.
    if (detent !== 'full') void nextTick(() => { if (sheetEl.value) sheetEl.value.scrollTop = 0; });
  },
);
