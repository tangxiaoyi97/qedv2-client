 ...
 * [0, 1] so malformed ratios can never corrupt the archive, then rounded to
 * 6 decimals at write time (checksum spec §5.1).
 */
export function updateMastery(prev: number | undefined, ratio: number): number {
  ...
  return normNum(Math.min(1, Math.max(0, next)));
}
